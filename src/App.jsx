...
export default function App() {
  ...
  // Refresh audit + chain when page changes
  useEffect(() => {
    setAudit(getAudit());
  ...
  const navigate = (p) => {
    if (p === "view") return;
    // Only update hash for pages that have dedicated hash routes
    const hashRoutes = { dashboard: "", create: "/create", blockchain: "/blockchain" };
    if (hashRoutes.hasOwnProperty(p)) {
      window.location.hash = hashRoutes[p];
    }
    // For pages without hash routes (notes, audit, security, settings)
    // just set the page state directly without changing hash
    setPage(p);
    setMobileOpen(false);
  };

  const refreshChain = () => setChainBlocks(loadChain().length);
    ...
    return (
      <>
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"100vh", gap:"1rem" }}>
          ...
          <div style={{ color:"var(--muted)", fontSize:".82rem" }}>Initializing blockchain ledger...</div>
        </div>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <div className="layout">
        ...
        <Sidebar page={page} navigate={navigate} notes={notes} audit={audit} chainBlocks={chainBlocks} mobileOpen={mobileOpen} />
        <div className="main">
          <Topbar page={page} navigate={navigate} onMenuToggle={() => setMobileOpen(!mobileOpen)} />
          <div className="content fade-in">
            {page === "dashboard"  && <DashboardPage notes={notes} audit={audit} navigate={navigate} chainBlocks={chainBlocks} />}
