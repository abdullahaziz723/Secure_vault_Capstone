...
export default function ViewPage({ noteId, refreshChain }) {
  ...
  const attemptDecrypt = async (note, pass, currentAttempts) => {
    ...
    try {
      ...
      // AI threat analysis — check for replay / harvesting patterns
      analyzeThreat({ type: "view", noteId: noteId.slice(0, 8) });
    } catch {
      const next = currentAttempts + 1;
      setAttempts(next);
      setError(`Incorrect password. Attempt ${next}/5.`);
      // AI threat analysis — brute force detection
      analyzeThreat({ type: "auth_fail", noteId: noteId.slice(0, 8) });
      if (next >= 5) {
